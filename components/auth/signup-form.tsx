...
function SelectField({ label, name, options }: { label: string; name: "zone"; options: string[] }) { const { register, formState: { errors } } = useFormContext<SignupValues>(); const message = errors[name]?.message; return <label className={labelClass}>{label} *<select {...register(name)} required aria-required="true" className={inputClass}><option value="">Select {label.toLowerCase()}</option>{options.map((option) => <option key={option} value={option}>{option}</option>)}</select>{typeof message === "string" && <span className="mt-1.5 block text-[10px] text-rose-600">{message}</span>}</label>; }
