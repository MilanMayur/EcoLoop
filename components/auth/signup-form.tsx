...
export function SignupForm() {
  ...
  const submit = async (values: SignupValues) => {
    ...
    setError(""); setSuccess("");
    try {
      const result = await authService.signup({ role, preferred_language: locale, ...Object.fromEntries(Object.entries(submissionValues).filter(([,value]) => value !== undefined).map(([key,value]) => [key, String(value)])) });
