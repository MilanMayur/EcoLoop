...
const signupSchema = z.object({
  ...
  company: z.string().trim().optional(), registration: z.string().trim().optional(), categories: z.array(z.enum(wasteTypeValues)).optional(), vehicles: z.string().trim().optional(),
...
const requiredRoleFields: Record<SignupRole, Array<{ name: keyof SignupValues; label: string }>> = {
  ...
  admin: [{ name: "name", label: "Officer name" }, { name: "employeeId", label: "Employee ID" }, { name: "zone", label: "TMC jurisdiction" }],
};
...
const roles = [
  ...
  { role: "admin" as const, icon: Building2, title: "TMC officer", description: "Monitor markets, partners, SLAs, and circularity outcomes.", detail: "Approval required" },
];
...
export function SignupForm() {
  ...
  const submit = async (values: SignupValues) => {
    if (!role) return;
    const submissionValues = role === "vendor" ? { ...values, market: "Chandapura" } : values;
    let hasMissingField = false;
    requiredRoleFields[role].forEach(({ name, label }) => {
      if (!String(submissionValues[name] ?? "").trim()) {
        form.setError(name, { type: "manual", message: `${label} is required.` });
        hasMissingField = true;
      }
    });
    if (role === "recycler" && submissionValues.vehicles && !/^[1-9]\d*$/.test(submissionValues.vehicles)) {
    ...
    setError(""); setSuccess("");
    try {
      const result = await authService.signup({ role, preferred_language: locale, ...Object.fromEntries(Object.entries(submissionValues).filter(([,value]) => value !== undefined).map(([key,value]) => [key, Array.isArray(value) ? value : String(value)])) });
      if (result.requiresEmailConfirmation) {
        setSuccess("Account created. Check your email to confirm it before signing in.");
        return;
      }
      if (result.requiresApproval) {
        setSuccess("Account created. A TMC administrator must approve access before you can sign in.");
        return;
      }
...
function RecyclerFields() { return <div className="grid gap-4 sm:grid-cols-2"><Field label="Company name" name="company" placeholder="GreenCycle Pvt Ltd" /><Field label="Registration number" name="registration" placeholder="KA-RCY-2026-0184" /><WasteCategoriesField /><Field label="Vehicle count" name="vehicles" type="number" placeholder="6" /><Field label="Phone" name="phone" type="tel" placeholder="+91 98765 43210" /><Field label="Email" name="email" type="email" placeholder="ops@greencycle.in" /><Field label="Password" name="password" type="password" placeholder="Create a strong password" helper={passwordHelp} /><Field label="Confirm password" name="confirmPassword" type="password" placeholder="Repeat password" /><div className="flex items-center gap-3 rounded-xl bg-emerald-50 p-3 sm:col-span-2"><Truck className="size-5 text-emerald-600" /><p className="text-[10px] leading-4 text-emerald-800">Vehicle and facility verification can be completed after signup. Special waste streams may require additional authorization.</p></div></div>; }
function WasteCategoriesField() { const { register, formState: { errors } } = useFormContext<SignupValues>(); return <fieldset className="sm:col-span-2"><legend className={labelClass}>Accepted waste streams *</legend><p className="mt-1 text-[10px] leading-4 text-slate-500">Select every category your company can collect. Specific materials can be refined during vehicle and facility verification.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{wasteStreams.map((stream) => <label key={stream.value} className="flex min-h-20 cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-white p-3 transition hover:border-emerald-300 has-[:checked]:border-emerald-500 has-[:checked]:bg-emerald-50"><input {...register("categories")} type="checkbox" value={stream.value} className="mt-1 size-4 shrink-0 accent-emerald-600" /><span><span className="flex items-center gap-2 text-xs font-semibold text-slate-800">{stream.label}{"restricted" in stream && stream.restricted && <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[8px] font-bold uppercase tracking-wider text-amber-700">Special handling</span>}</span><span className="mt-1 block text-[10px] font-normal leading-4 text-slate-500">{stream.description}</span></span></label>)}</div>{typeof errors.categories?.message === "string" && <span className="mt-1.5 block text-[10px] text-rose-600">{errors.categories.message}</span>}</fieldset>; }
function AdminFields() { return <div><div className="grid gap-4 sm:grid-cols-2"><Field label="Officer name" name="name" placeholder="Priya Nair" /><Field label="Employee ID" name="employeeId" placeholder="TMC-18472" /><Field label="TMC jurisdiction" name="zone" placeholder="Chandapura TMC" /><Field label="Phone" name="phone" type="tel" placeholder="+91 98765 43210" /><Field label="Official email" name="email" type="email" placeholder="priya.nair@tmc.gov.in" /><Field label="Password" name="password" type="password" placeholder="Create a strong password" helper={passwordHelp} /><Field label="Confirm password" name="confirmPassword" type="password" placeholder="Repeat password" /></div><div className="mt-5 flex gap-3 rounded-xl border border-blue-200 bg-blue-50 p-4"><ShieldCheck className="size-5 shrink-0 text-blue-600" /><div><p className="text-xs font-semibold text-blue-900">Super Admin approval required</p><p className="mt-1 text-[10px] leading-4 text-blue-700">Your official credentials will be reviewed before live operational access is enabled.</p></div></div></div>; }
function Field({ label, name, type = "text", placeholder, helper }: { label: string; name: keyof SignupValues; type?: string; placeholder: string; helper?: string }) { const { register, formState: { errors } } = useFormContext<SignupValues>(); const message = errors[name]?.message; const isFixedVendorMarket = name === "market"; return <label className={labelClass}>{label} *<input {...register(name)} type={type} placeholder={placeholder} value={isFixedVendorMarket ? "Chandapura" : undefined} readOnly={isFixedVendorMarket} aria-readonly={isFixedVendorMarket || undefined} required aria-required="true" className={cn(inputClass, isFixedVendorMarket && "cursor-not-allowed bg-slate-100 text-slate-500")} />{helper && <span className="mt-1.5 block text-[10px] font-normal leading-4 text-slate-400">{helper}</span>}{typeof message === "string" && <span className="mt-1.5 block text-[10px] text-rose-600">{message}</span>}</label>; }
