...
export function LoginForm() {
  ...
  return <div className="w-full"><p className="text-[11px] font-bold uppercase tracking-[.14em] text-emerald-600">Welcome back</p><h1 className="mt-2 text-3xl font-semibold tracking-[-.05em] text-slate-950 sm:text-4xl">Sign in to EcoLoop</h1><p className="mt-3 text-sm leading-6 text-slate-500">Access the workspace tailored to your role.</p><form onSubmit={loginForm.handleSubmit(submit)} className="mt-8 space-y-5" noValidate><label className="block text-xs font-semibold text-slate-700">Email address<div className="relative mt-2"><Mail className="absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><input {...loginForm.register("email")} type="email" autoComplete="email" aria-invalid={Boolean(loginForm.formState.errors.email)} className="h-12 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10" /></div>{loginForm.formState.errors.email && <span className="mt-1.5 block text-[10px] text-rose-600">{loginForm.formState.errors.email.message}</span>}</label><PasswordField label="Password" registration={loginForm.register("password")} error={loginForm.formState.errors.password?.message} show={show} onToggle={() => setShow(!show)} autoComplete="current-password" /><div className="flex items-center justify-between"><label className="flex items-center gap-2 text-xs text-slate-500"><input {...loginForm.register("remember")} type="checkbox" className="size-4 rounded accent-emerald-600" /> Remember me</label><button type="button" disabled={resetting || loginForm.formState.isSubmitting} onClick={forgotPassword} className="text-xs font-semibold text-emerald-600 disabled:text-slate-400">{resetting ? "Sending reset email…" : "Forgot password?"}</button></div>{error && <p role="alert" className="rounded-xl bg-rose-50 px-4 py-3 text-xs text-rose-700">{error}</p>}<Button className="h-12 w-full" type="submit" disabled={loginForm.formState.isSubmitting || resetting}>{loginForm.formState.isSubmitting ? "Signing in…" : "Sign in"}<ArrowRight className="size-4" /></Button></form><div className="my-6 flex items-center gap-3"><span className="h-px flex-1 bg-slate-200" /><span className="text-[10px] uppercase tracking-wider text-slate-400">or</span><span className="h-px flex-1 bg-slate-200" /></div><Button variant="outline" className="h-12 w-full" disabled title="Coming Soon"><span className="text-sm font-bold text-slate-400">G</span> Google sign-in · Coming Soon</Button><p className="mt-7 text-center text-xs text-slate-500">Don’t have an account? <Link href="/signup" className="font-semibold text-emerald-600 hover:text-emerald-700">Create one</Link></p>{notice && <AuthToast message={notice} onClose={() => setNotice("")} />}</div>;
}
...
function PasswordField({ label, registration, error, show, onToggle, autoComplete }: { label: string; registration: UseFormRegisterReturn; error?: string; show: boolean; onToggle: () => void; autoComplete: string }) {
  return <label className="block text-xs font-semibold text-slate-700">{label}<div className="relative mt-2"><LockKeyhole className="absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-slate-400" /><input {...registration} type={show ? "text" : "password"} autoComplete={autoComplete} aria-invalid={Boolean(error)} className="h-12 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-11 text-sm outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10" /><button type="button" onClick={onToggle} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400" aria-label={show ? "Hide password" : "Show password"}>{show ? <EyeOff className="size-4" /> : <Eye className="size-4" />}</button></div>{error && <span className="mt-1.5 block text-[10px] text-rose-600">{error}</span>}</label>;
}
