...
export function LoginForm() {
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      if (linkError || errorCode) {
        const expired = `${linkError ?? ""} ${errorCode ?? ""}`
          .toLowerCase()
          .includes("expired");
        setError(
          expired
            ? "This verification or password-reset link has expired. Request a new one."
            : decodeURIComponent(
                (linkError ?? "The authentication link is invalid.").replace(
                  /\+/g,
                  " ",
                ),
              ),
      ...
      if (query.get("recovery") === "1" || query.get("invited") === "1")
    ...
    try {
      subscription = authService.onAuthStateChange((event) => {
        ...
        if (event === "SIGNED_IN" && query.get("verified") === "1") {
          setNotice(
            "Email verified successfully. Your EcoLoop account is ready.",
          );
        }
      });
    } catch (caught) {
  ...
  const forgotPassword = async () => {
    ...
    const parsed = z
      ...
      .safeParse(loginForm.getValues("email"));
    if (!parsed.success) {
      setError(
  ...
  if (recoveryMode) {
    return (
      <div className="w-full">
        <p className="text-[11px] font-bold uppercase tracking-[.14em] text-emerald-600">
          Account recovery
        </p>
        ...
        <p className="mt-3 text-sm leading-6 text-slate-500">
          Choose a secure password for your EcoLoop account.
        </p>
        <form
          onSubmit={recoveryForm.handleSubmit(updatePassword)}
        ...
        >
          <PasswordField
            ...
            error={recoveryForm.formState.errors.password?.message}
            show={show}
          ...
          <PasswordField
            ...
            registration={recoveryForm.register("confirmPassword")}
            error={recoveryForm.formState.errors.confirmPassword?.message}
            show={show}
            onToggle={() => setShow(!show)}
            autoComplete="new-password"
          />
        ...
        {notice && <AuthToast message={notice} onClose={() => setNotice("")} />}
      </div>
    );
  }
  ...
  return (
    <div className="w-full">
      ...
      <p className="mt-3 text-sm leading-6 text-slate-500">
        Access the workspace tailored to your role.
      </p>
      <form
      ...
      >
        <label className="block text-xs font-semibold text-slate-700">
          ...
          <div className="relative mt-2">
            ...
            <input
              ...
              className="h-12 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-base outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 sm:text-sm"
            />
          </div>
        ...
        <div className="flex flex-col gap-1 min-[430px]:flex-row min-[430px]:items-center min-[430px]:justify-between">
          ...
          <button
            type="button"
            disabled={resetting || loginForm.formState.isSubmitting}
            onClick={forgotPassword}
            className="min-h-11 self-start text-xs font-semibold text-emerald-600 disabled:text-slate-400"
          >
        ...
        <Button
          className="h-12 w-full"
          type="submit"
          disabled={loginForm.formState.isSubmitting || resetting}
        >
      ...
      <Button
        ...
        title="Coming Soon"
      >
        <span className="text-sm font-bold text-slate-400">G</span> Google
        sign-in · Coming Soon
      </Button>
      <p className="mt-5 text-center text-xs text-slate-500 sm:mt-7">
        ...
        <Link
          href="/signup"
          className="inline-flex min-h-11 items-center font-semibold text-emerald-600 hover:text-emerald-700"
        >
          Create one
        </Link>
      </p>
    ...
    <label className="block text-xs font-semibold text-slate-700">
      ...
      <div className="relative mt-2">
        ...
        <input
          ...
          className="h-12 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-12 text-base outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 sm:text-sm"
        />
        <button
          type="button"
          onClick={onToggle}
