...
import { wasteTypeValues, type WasteType } from "@/lib/waste-taxonomy";
...
const adminSettingsSchema = z.object({
  name: z.string().min(2, "Enter your full name."),
  market: z.string().min(2, "Enter your jurisdiction or operating area."),
  email: z.string().email("Enter a valid email address."),
  phone: z.string().min(10, "Enter a valid phone number."),
});
...
function AdminSettingsPage() {
  ...
  const displayName = current?.name || "TMC account";
  const imageUrl = profileImagePreview || current?.profileImageUrl;
  const initials = displayName.split(/\s+/).slice(0, 2).map((part) => part[0]).join("").toUpperCase() || "TA";
      ...
      <form onSubmit={handleSubmit(submit)} className="grid gap-3 sm:gap-5 xl:grid-cols-[.7fr_1.3fr]" noValidate>
        <Panel>
          <div className="p-4 text-center sm:p-6">
            ...
            <p className="mt-1 text-xs text-slate-500">{current?.market || "TMC administrator"}</p>
        ...
        <div className="space-y-3 sm:space-y-5">
          <Panel title="Profile details">
            <div className="grid gap-3 p-4 sm:grid-cols-2 sm:gap-4 sm:p-6">
              ...
              <label className={labelClass}>Jurisdiction or operating area<input {...register("market")} className={`${inputClass} mt-1.5 h-10 text-sm sm:mt-2 sm:h-11`} />{errors.market && <span className="mt-1 block text-[10px] text-rose-600 sm:mt-1.5">{errors.market.message}</span>}</label>
...
function ProfilePage({ role, settings = false }: { role: DashboardRole; settings?: boolean }) {
  ...
  const displayOrganization = role === "admin" ? current?.market || "TMC administrator" : current?.organization || current?.market || roleProfiles[role].shortRole;
  const imageUrl = profileImagePreview || current?.profileImageUrl;
