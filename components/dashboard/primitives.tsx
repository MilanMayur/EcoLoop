...
export function StatusBadge({ status }: { status: string }) {
  const style = status === "Cancelled" ? "bg-rose-50 text-rose-700 ring-rose-600/15 dark:bg-rose-500/10 dark:text-rose-300" : status === "Completed" || status === "Healthy" || status === "Active" || status === "Available" ? "bg-emerald-50 text-emerald-700 ring-emerald-600/15 dark:bg-emerald-500/10 dark:text-emerald-400" : status === "In transit" || status === "On route" || status === "Assigned" || status === "Accepted" || status === "Arrived" || status === "Collected" ? "bg-blue-50 text-blue-700 ring-blue-600/15 dark:bg-blue-500/10 dark:text-blue-400" : status === "Attention" || status === "High" || status === "Delayed" || status === "Batching" || status === "On break" ? "bg-amber-50 text-amber-700 ring-amber-600/15 dark:bg-amber-500/10 dark:text-amber-400" : "bg-slate-100 text-slate-600 ring-slate-500/15 dark:bg-slate-800 dark:text-slate-300";
