...
export function WasteTrendChart({ mode = "bar", compactOnMobile = false }: { mode?: "bar" | "line"; compactOnMobile?: boolean }) {
  const { data, loading, error } = useAsyncResource(() => analyticsService.getCharts(), "dashboard-charts");
  if (loading) return <ChartState label="Loading chart…" loading compactOnMobile={compactOnMobile} />;
  if (error || !data) return <ChartState label="Chart data is unavailable." error compactOnMobile={compactOnMobile} />;
  if (!data.wasteTrend.some((point) => point.collected > 0 || point.recycled > 0)) return <ChartState label="No measured pickup data yet." compactOnMobile={compactOnMobile} />;
  return (
    <div className={`${compactOnMobile ? "h-48 sm:h-64" : "h-64"} min-w-0 w-full`} aria-label="Monthly waste collection and recycling chart">
...
export function WasteDonutChart({ compactOnMobile = false }: { compactOnMobile?: boolean }) {
  const { data, loading, error } = useAsyncResource(() => analyticsService.getCharts(), "dashboard-charts");
  if (loading) return <ChartState label="Loading chart…" loading compactOnMobile={compactOnMobile} />;
  if (error || !data) return <ChartState label="Chart data is unavailable." error compactOnMobile={compactOnMobile} />;
  if (!data.wasteCategories.length) return <ChartState label="No measured material data yet." compactOnMobile={compactOnMobile} />;
  return (
    <div className="flex flex-col items-center gap-2 sm:flex-row sm:gap-3" aria-label="Waste category distribution chart">
      <div className={`${compactOnMobile ? "h-40 sm:h-52" : "h-52"} w-full min-w-0 flex-1`}><ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}><PieChart><Pie data={data.wasteCategories} dataKey="value" nameKey="name" innerRadius={56} outerRadius={80} paddingAngle={3} stroke="none">{data.wasteCategories.map((entry) => <Cell key={entry.name} fill={entry.color} />)}</Pie><Tooltip contentStyle={tooltipStyle} /></PieChart></ResponsiveContainer></div>
      <div className="grid min-w-32 grid-cols-2 gap-x-4 gap-y-2 sm:grid-cols-1 sm:gap-x-5 sm:gap-y-3">{data.wasteCategories.map(item => <div key={item.name} className="flex items-center gap-1.5 text-[10px] text-slate-500 sm:gap-2 sm:text-xs"><span className="size-2 rounded-full" style={{ background: item.color }} /><span>{item.name}</span><span className="ml-auto font-semibold text-slate-700 dark:text-slate-200">{item.value}%</span></div>)}</div>
    </div>
  );
}

function ChartState({ label, error = false, loading = false, compactOnMobile = false }: { label: string; error?: boolean; loading?: boolean; compactOnMobile?: boolean }) {
  return <div className={`grid ${compactOnMobile ? "h-48 sm:h-64" : "h-64"} place-items-center rounded-xl text-xs ${error ? "bg-rose-50 text-rose-600 dark:bg-rose-500/10" : loading ? "animate-pulse bg-slate-50 text-slate-400 dark:bg-slate-800" : "bg-slate-50 text-slate-500 dark:bg-slate-800"}`}>{label}</div>;
}
