...
export function SmartStockNavigation() {
  ...
  return <nav aria-label="Smart Stock navigation" className="-mx-0.5 sm:-mx-6 sm:overflow-x-auto sm:px-6 lg:-mx-8 lg:px-8"><div className="grid grid-cols-4 gap-0.5 rounded-xl border border-slate-200 bg-white p-0.5 sm:flex sm:min-w-max sm:gap-1 sm:p-1 dark:border-slate-800 dark:bg-slate-900">{items.map(item => { const active = pathname === item.href; return <Link key={item.href} href={item.href} className={cn("flex min-h-10 min-w-0 flex-col items-center justify-center gap-0.5 rounded-lg px-1 py-1.5 text-center text-[8px] font-semibold leading-3 transition sm:min-h-0 sm:flex-row sm:gap-2 sm:px-4 sm:py-2 sm:text-[11px]", active ? "bg-slate-950 text-white shadow-sm dark:bg-emerald-600" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-white")}><item.icon className="size-3.5" />{item.label}</Link>; })}</div></nav>;
}

export function SmartStockModuleShell({ children }: { children: React.ReactNode }) {
  return <div className="space-y-2.5 sm:space-y-6"><div className="flex items-center gap-2 sm:gap-3"><span className="grid size-8 sm:size-10 place-items-center rounded-xl bg-emerald-600 text-white shadow-[0_8px_22px_rgba(22,163,74,.2)]"><PackageSearch className="size-4 sm:size-[18px]" /></span><div><p className="text-[9px] font-bold uppercase tracking-[.16em] text-emerald-600 dark:text-emerald-400">AI-powered waste prevention</p><p className="text-sm font-semibold text-slate-900 dark:text-white">EcoLoop Smart Stock</p></div><span className="ml-auto hidden rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-[9px] font-bold uppercase tracking-wider text-emerald-700 dark:border-emerald-900 dark:bg-emerald-500/10 dark:text-emerald-400 sm:block">Forecast updated 8 min ago</span></div><SmartStockNavigation />{children}</div>;
}
