...
function ChartState({ error = false }: { error?: boolean }) {
  return <div className={`grid h-44 place-items-center rounded-xl sm:h-64 text-xs ${error ? "bg-rose-50 text-rose-600 dark:bg-rose-500/10" : "animate-pulse bg-slate-50 text-slate-400 dark:bg-slate-800"}`}>{error ? "Chart data is unavailable." : "Loading chart…"}</div>;
}
