...
export function InventoryDemandChart() {
  ...
  if (!resource.data.inventoryDemand.length) return <ChartState message="Add inventory items to compare current stock with forecast demand." />;
...
export function TopWasteChart() {
  const resource = useAsyncResource(() => analyticsService.getSmartStockAnalytics(), "smart-stock-charts");
  if (resource.loading || resource.error || !resource.data) return <ChartState error={Boolean(resource.error)} />;
  if (!resource.data.topWaste.some((item) => item.potential > 0)) return <ChartState message="No products are currently above forecast demand." />;
...
function ChartState({ error = false, message }: { error?: boolean; message?: string }) {
  const loading = !error && !message;
  return <div className={`grid h-44 place-items-center rounded-xl px-6 text-center text-xs sm:h-64 ${error ? "bg-rose-50 text-rose-600 dark:bg-rose-500/10" : loading ? "animate-pulse bg-slate-50 text-slate-400 dark:bg-slate-800" : "bg-slate-50 text-slate-500 dark:bg-slate-800"}`}>{error ? "Chart data is unavailable." : message ?? "Loading chart…"}</div>;
}
