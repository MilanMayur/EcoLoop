...
const rolePayloadSchema = z.object({ role: z.enum(["vendor", "recycler", "admin"]), context: z.unknown() });
const reportPayloadSchema = rolePayloadSchema.extend({ period: z.enum(["weekly", "monthly"]) });
const chatPayloadSchema = z.object({ role: z.enum(["vendor", "recycler", "admin"]), message: z.string().trim().min(1).max(800), history: z.array(z.object({ role: z.enum(["user", "assistant"]), content: z.string().max(1200) })).max(10).optional(), context: z.unknown().optional() });
