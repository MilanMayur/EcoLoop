...
alter table public.pickup_requests
  drop constraint if exists pickup_requests_cancellation_reason_check;

alter table public.pickup_requests
  add constraint pickup_requests_cancellation_reason_check
  check (
    cancellation_reason is null
    or char_length(trim(cancellation_reason)) between 5 and 500
  );

create index if not exists pickup_requests_cancelled_idx
  on public.pickup_requests(cancelled_at desc)
  where status = 'cancelled';

create or replace function public.enforce_pickup_cancellation()
returns trigger
language plpgsql
set search_path = ''
as $$
declare
  cancellation_context text;
begin
  if new.status = 'cancelled' and old.status is distinct from 'cancelled' then
    cancellation_context := current_setting(
      'ecoloop.pickup_cancellation_context',
      true
    );

    if coalesce(cancellation_context, '') not in ('vendor', 'recycler', 'admin') then
      raise exception 'Use the dedicated pickup cancellation action';
    end if;
    ...
    if new.cancelled_at is null
      ...
      or new.cancelled_by_role is null then
      raise exception 'Cancellation audit details are required';
    end if;
  end if;
...
drop trigger if exists enforce_pickup_cancellation on public.pickup_requests;
create trigger enforce_pickup_cancellation
before update of status on public.pickup_requests
for each row execute function public.enforce_pickup_cancellation();
...
begin
  ...
  if actor_role = 'vendor' then
    ...
    if current_row.status not in (
      ...
      raise exception 'BBMP cannot cancel at this pickup stage';
    end if;
    if resolved_reason is null then
      raise exception 'BBMP must provide an exceptional cancellation reason';
    end if;
  else
    raise exception 'Only vendors, recycler managers, or BBMP can cancel pickups';
  end if;
  ...
  if result.recycler_id is not null and actor_role <> 'recycler' then
    ...
    values (
      ...
      'recycler',
      'Pickup cancelled',
      result.reference_code || ' was cancelled by BBMP. Reason: ' || resolved_reason,
      'pickup',
      result.id
    );
  end if;
